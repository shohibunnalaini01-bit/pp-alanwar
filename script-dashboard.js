...
async function loadDataMurid() {
    ...
    try {
        const response = await fetch(`${BASE_URL}?select=*&order=nome_murid.asc`, { headers: HEADERS_GET });
...
function renderTable(data) {
    ...
    data.forEach(m => {
        tbody.innerHTML += `
            <tr>
                <td style="cursor:pointer;color:#00703c;font-weight:bold;" onclick="lihatDetail(${m.id})">${m.nome_murid || m.nama_murid || '-'}</td>
                <td>${m.nik_murid || '-'}</td>
                <td>${m.desa || '-'}</td>
                <td>
                    <button onclick="lihatDetail(${m.id})" style="color:blue;border:none;background:none;cursor:pointer;"><i class="fas fa-eye"></i></button>
                    <button onclick="hapusData(${m.id}, '${m.nome_murid || m.nama_murid}')" style="color:red;border:none;background:none;cursor:pointer;"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`;
    });
}
...
function searchData() {
    ...
    renderTable(window.allDataMurid.filter(m => (m.nome_murid || m.nama_murid).toLowerCase().includes(keyword) || m.nik_murid.includes(keyword)));
}
...
document.getElementById('formMurid').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        else { const err = await response.json(); Swal.fire('Gagal!', err.message || 'Tidak bisa menyimpan data.', 'error'); }
...
async function lihatDetail(id) {
    ...
    try {
        ...
        htmlDetail += `<h4 style="margin-bottom:5px;">Data Pribadi</h4>
            ...
            <b>Ayah:</b> ${m.nome_ayah || m.nome_ayah || '-'}<br><b>Ibu:</b> ${m.nome_ibu || m.nome_ibu || '-'}<br>
...
async function refreshDropdownKelas() {
    try {
        ...
        data.forEach(k => { select.innerHTML += `<option value="${k.id}">${k.nome_kelas || k.nome_kelas}</option>`; });
...
async function tambahMuridKeKelas() {
    ...
    try {
        ...
        const options = {};
        muridTersedia.forEach(m => { options[m.id] = `${m.nome_murid || m.nome_murid} (${m.desa || '-'})`; });
...
// DIPERBAIKI: Menggunakan nome_kelas agar masuk ke database
async function tambahNomeKelasMaster() {
    ...
    try {
        ...
        else { const err = await res.json(); Swal.fire('Gagal Database', err.message, 'error'); }
...
async function refreshDropdownKamar() {
    try {
        ...
        data.forEach(k => { select.innerHTML += `<option value="${k.id}">${k.nome_kamar || k.nome_kamar}</option>`; });
    } catch (err) { console.error(err); }
}

// DIPERBAIKI: Menambah tombol Cabut Ketua Kamar
async function loadPenghuniKamar() {
    ...
    try {
        ...
        data.forEach(m => {
            ...
            const nomeTampil = isKetua ? `<span style="color:#d35400; font-weight:bold;">⭐ ${m.nome_murid || m.nome_murid} (Ketua)</span>` : (m.nome_murid || m.nome_murid);
            ...
            if (isKetua) {
                ...
                btnAksi = `<button onclick="jadikanKetuaKamar(${m.id}, ${idKamar})" class="btn-keluarkan" style="border-color:blue;color:blue;"><i class="fas fa-star"></i> Ketua</button> `;
            }
...
async function tambahMuridKeKamar() {
    ...
    const options = {};
    murid.forEach(m => { options[m.id] = m.nome_murid || m.nome_murid; });
...
// DIPERBAIKI: Menggunakan nome_kamar agar masuk ke database
async function tambahNomeKamarMaster() {
    ...
    try {
        ...
        if (res.ok) { Swal.fire('Berhasil', 'Kamar terdaftar', 'success'); refreshDropdownKamar(); refreshDataDisiplin(); }
        else { const err = await res.json(); Swal.fire('Gagal Database', err.message, 'error'); }
    } catch(e) { Swal.fire('Gagal', 'Koneksi database error.', 'error'); }
}

/* =========================
   5. PERIZINAN (POP-UP FILTER KELAS)
========================= */
...
// DIPERBAIKI: Fungsi ini dipanggil dari HTML (onclick)
async function pilihSantriIzin() {
    try {
        const resKelas = await fetch(URL_KELAS, { headers: HEADERS_GET });
        const dataKelas = await resKelas.json();
        const optionsKelas = {};
        dataKelas.forEach(k => { optionsKelas[k.id] = k.nome_kelas || k.nome_kelas; });

        const { value: idKelas } = await Swal.fire({ title: 'Pilih Kelas Terlebih Dahulu', input: 'select', inputOptions: optionsKelas, showCancelButton: true, confirmButtonText: 'Lihat Santri', confirmButtonColor: '#00703c' });
        if (!idKelas) return;
        ...
        const optionsMurid = {};
        dataMurid.forEach(m => { optionsMurid[m.id] = `${m.nome_murid || m.nome_murid} (${m.desa || '-'})`; });

        const { value: idMurid } = await Swal.fire({ title: 'Pilih Santri', input: 'select', inputOptions: optionsMurid, showCancelButton: true, confirmButtonText: 'Pilih', confirmButtonColor: '#00703c' });
        if (idMurid) {
            const muridTerpilih = dataMurid.find(m => m.id == idMurid);
            document.getElementById('pilih-santri-izin-text').value = muridTerpilih.nome_murid || muridTerpilih.nome_murid;
...
async function simpanIzin() {
    ...
    try {
        ...
        } else { const err = await res.json(); Swal.fire('Gagal', err.message, 'error'); }
    } catch (err) { console.error(err); }
}
...
async function loadDataIzin(filter) {
    ...
    try {
        ...
        data.forEach(item => {
            ...
            tbody.innerHTML += `
                <tr>
                    <td><b>${item.murid.nome_murid || item.murid.nome_murid}</b><br><small>${item.murid.desa}</small></td>
                    <td>${item.alasan}</td>
...
async function refreshDataDisiplin() {
    ...
    try {
        ...
        let rekapPoin = {};
        let ketuaKamarMap = {};
        allMurid.forEach(m => { if(m.is_ketua_kamar && m.id_kamar) ketuaKamarMap[String(m.id_kamar)] = m.nome_murid || m.nome_murid; });

        if (menuAktifDisiplin === 'kelas') {
            dataKelas.forEach(k => { rekapPoin[String(k.id)] = { nome: k.nome_kelas || k.nome_kelas, poin: 0 }; });
            thead.innerHTML = '<tr><th>Rank</th><th>Nama Kelas</th><th>Total Poin</th></tr>';
        } else {
            dataKamar.forEach(k => { rekapPoin[String(k.id)] = { nome: k.nome_kamar || k.nome_kamar, poin: 0 }; });
...
async function pilihMuridBerat() {
    try {
        ...
        if (idMurid) {
            const muridTerpilih = dataMurid.find(m => m.id == idMurid);
            document.getElementById('pilih-murid-berat-text').value = muridTerpilih.nome_murid || muridTerpilih.nome_murid;
...
async function simpanPelanggaranBerat() {
    ...
    try {
        const res = await fetch(URL_PELANGGARAN, {
            ...
            body: JSON.stringify({ id_murid: parseInt(id_murid), tipo_pelanggaran: tipo_pelanggaran, poin: parseInt(poin), tgl_kejadian: new Date().toISOString().split('T')[0], kategori: "Berat" })
        });
        if (res.ok) {
        ...
        } else { const errData = await res.json(); Swal.fire('Gagal', errData.message, 'error'); }
...
async function tampilkanMenuTombolKelas() {
    ...
    try {
        ...
        container.innerHTML = '';
        data.forEach(k => {
            container.innerHTML += `<div class="card-kelas" onclick="bukaFormMassal(${k.id}, '${k.nome_kelas || k.nome_kelas}')"><i class="fas fa-users"></i><h4>${k.nome_kelas || k.nome_kelas}</h4><p>Klik untuk isi audit</p></div>`;
        });
        ...
        dataMurid.forEach(m => {
            tbody.innerHTML += `
                <tr class="row-rekap-santri" data-id="${m.id}">
                    <td><b>${m.nome_murid || m.nome_murid}</b></td>
...
async function inisialisasiGrafikKelas() {
    ...
    try {
        ...
        daftarKelas.forEach(k => { labelKelas.push(k.nome_kelas || k.nome_kelas); dataJumlah.push(daftarMurid.filter(m => m.id_kelas === k.id).length); });
...
async function exportToExcel(type) {
    ...
    let dataToExport = []; let filename = "Export_Data.xlsx"; let sheetName = "Data";

    try {
        const [resMurid, resKelas, resKamar] = await Promise.all([
            fetch(`${BASE_URL}?select=id,nome_murid,nik_murid,id_kelas,id_kamar,desa`, { headers: HEADERS_GET }),
            fetch(URL_KELAS, { headers: HEADERS_GET }),
            fetch(URL_KAMAR, { headers: HEADERS_GET })
        ]);
        const allMurid = await resMurid.json(); const allKelas = await resKelas.json(); const allKamar = await resKamar.json();
        const kelasMap = {}; allKelas.forEach(k => kelasMap[k.id] = k.nome_kelas || k.nome_kelas);
        const kamarMap = {}; allKamar.forEach(k => kamarMap[k.id] = k.nome_kamar || k.nome_kamar);
        const muridMap = {}; allMurid.forEach(m => muridMap[m.id] = m);

        if(type === 'murid') {
            allMurid.forEach(m => dataToExport.push({ "Nome Murid": m.nome_murid || m.nome_murid, "NIK": m.nik_murid, "Desa": m.desa, "Kelas": kelasMap[m.id_kelas] || '-', "Kamar": kamarMap[m.id_kamar] || '-' }));
            ...
            const [resRingan, resBerat] = await Promise.all([ fetch(URL_REKAP, { headers: HEADERS_GET }), fetch(URL_PELANGGARAN, { headers: HEADERS_GET }) ]);
            const ringan = await resRingan.json(); const berat = await resBerat.json();
            ringan.forEach(r => {
                const murid = muridMap[r.id_murid]; if(!murid) return;
                const grupNome = isKamar ? (kamarMap[murid.id_kamar] || '-') : (kelasMap[murid.id_kelas] || '-');
                if(isKamar && !murid.id_kamar) return; if(!isKamar && !murid.id_kelas) return;
                dataToExport.push({ [`Nome ${isKamar ? 'Kamar' : 'Kelas'}`]: grupNome, "Nome Murid": murid.nome_murid || murid.nome_murid, "Bulan": r.bulan, "Minggu": r.minggu, "Jamaah": r.jamaah, "Batin": r.gerak_batin, "Sekolah": r.sekolah, "Sunnah": r.sholat_sunnah, "Kategori": "Ringan" });
            });
            berat.forEach(b => {
                const murid = muridMap[b.id_murid]; if(!murid) return;
                const grupNome = isKamar ? (kamarMap[murid.id_kamar] || '-') : (kelasMap[murid.id_kelas] || '-');
                if(isKamar && !murid.id_kamar) return; if(!isKamar && !murid.id_kelas) return;
                dataToExport.push({ [`Nome ${isKamar ? 'Kamar' : 'Kelas'}`]: grupNome, "Nome Murid": murid.nome_murid || murid.nome_murid, "Bulan": b.tgl_kejadian ? new Date(b.tgl_kejadian).getMonth()+1 : '-', "Minggu": "-", "Jenis Pelanggaran": b.jenis_pelanggaran, "Poin": b.poin, "Kategori": "Berat" });
            });
            filename = `Pelanggaran_Per${isKamar ? 'Kamar' : 'Kelas'}.xlsx`; sheetName = `Pelanggaran ${isKamar ? 'Kamar' : 'Kelas'}`;
        }
        else if(type === 'rekap_total') {
            const res = await fetch(URL_REKAP, { headers: HEADERS_GET }); const rekap = await res.json();
            rekap.forEach(r => { const murid = muridMap[r.id_murid]; if(!murid) return; dataToExport.push({ "Nome Murid": murid.nome_murid || murid.nome_murid, "Kelas": kelasMap[murid.id_kelas] || '-', "Kamar": kamarMap[murid.id_kamar] || '-', "Bulan": r.bulan, "Minggu": r.minggu, "Jamaah": r.jamaah, "Batin": r.gerak_batin, "Sekolah": r.sekolah, "Sunnah": r.sholat_sunnah }); });
            filename = "Rekap_Total_Pelanggaran.xlsx"; sheetName = "Rekap Total";
        }
        else if(type === 'perizinan') {
            const res = await fetch(`${URL_IZIN}?select=*&order=tgl_pulang.desc`, { headers: HEADERS_GET }); const izinRaw = await res.json();
            izinRaw.forEach(i => { const murid = muridMap[i.id_murid]; if(!murid) return; dataToExport.push({ "Nome Murid": murid.nome_murid || murid.nome_murid, "Desa": murid.desa, "Alasan": i.alasan, "Status": i.status, "Tanggal Pulang": i.tgl_pulang ? new Date(i.tgl_pulang).toLocaleString('id-ID') : '-', "Rencana Kembali": i.tgl_kembali_rencana ? new Date(i.tgl_kembali_rencana).toLocaleString('id-ID') : 'Sampai Sembuh', "Estimasi Hari": i.estimasi_hari || '-' }); });
            filename = "Data_Perizinan.xlsx"; sheetName = "Perizinan";
        }
        const ws = XLSX.utils.json_to_sheet(dataToExport); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, sheetName); XLSX.writeFile(wb, filename); Swal.close();
    } catch(e) { console.error(e); Swal.fire('Gagal Export', 'Terjadi kesalahan saat mengambil data dari server.', 'error'); }
}
...
function exportPelanggaranExcel() {
    Swal.fire({ title: 'Pilih Jenis Export Pelanggaran', input: 'select', inputOptions: { 'pelanggaran_kamar': 'Pelanggaran Per Kamar', 'pelanggaran_kelas': 'Pelanggaran Per Kelas', 'rekap_total': 'Rekap Seluruh Pelanggaran' }, inputPlaceholder: 'Pilih jenis laporan', showCancelButton: true, confirmButtonText: 'Export', confirmButtonColor: '#008f4c', preConfirm: (value) => { if (!value) { Swal.showValidationMessage('Anda harus memilih jenis laporan!'); } return value; } }).then((result) => { if (result.isConfirmed) { exportToExcel(result.value); } });
}
...
function initApp() {
    ...
    document.querySelectorAll('.nav-menu li').forEach(li => {
        ...
        if (link) { const page = link.getAttribute('data-page'); if (page && !allowedPages.includes(page)) li.style.display = 'none'; else if (page) li.style.display = ''; }
    });
    let defaultLink = document.querySelector(`.nav-menu a[data-page="ringkasan"]`);
    showPage('ringkasan', defaultLink);
    loadDataMurid(); refreshDropdownKelas(); refreshDropdownKamar();
